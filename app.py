...
def generate_pdf_cohen_tomador(mes, dia, cliente, interes, prestamista, comitente_prestamista, depositante_prestamista, tomador, comitente_tomador, depositante_tomador, especie, codigo_especie, valor_nominal, tasa_anual, plazo, cuenta_bancaria, cuit, domicilio):
    ...
    body = (f"Ciudad Autónoma de Buenos Aires, {sanitize_text(mes)} de {sanitize_text(dia)} de 2024\n\n"
            ...
            f"Interés: Se calculará sobre el valor promedio de cierre de contado 48 horas de los Valores Negociables por el Plazo pactado. Será abonado en pesos desde la Cuenta del Tomador mediante transferencia a la Cuenta del Prestamista. El mismo será calculado de acuerdo al Tipo de Cambio [ ] del día de pago.\n"
...
def generate_pdf_cohen_tomador_tbills(mes, dia, cliente, interes, prestamista, comitente_prestamista, depositante_prestamista, tomador, comitente_tomador, depositante_tomador, especie, codigo_especie, valor_nominal, tasa_anual, plazo, cuenta_bancaria, cuit, domicilio):
    pdf = PDF()
    pdf.add_page()
    pdf.set_left_margin(10)
    pdf.set_right_margin(10)
...
def generate_pdf_cohen_prestamista_tbills(mes, dia, cliente, interes, prestamista, comitente_prestamista, depositante_prestamista, tomador, comitente_tomador, depositante_tomador, especie, codigo_especie, valor_nominal, tasa_anual, plazo, cuenta_bancaria, cuit, domicilio):
    pdf = PDF()
    pdf.add_page()
    pdf.set_left_margin(10)
    pdf.set_right_margin(10)
...
def generate_pdf_prestamo_entre_clientes(mes, dia, prestamista, tomador, cuit_prestamista, cuit_tomador, domicilio_prestamista, domicilio_tomador, especie, valor_nominal, tasa_anual, plazo, tasa_penalidad):
    pdf = PDF()
    pdf.add_page()
    pdf.set_left_margin(10)
    pdf.set_right_margin(10)
    ...
    body = (f"Ciudad Autónoma de Buenos Aires, {sanitize_text(dia)} de {sanitize_text(mes)} de 2024\n\n"
            f"Sres.\n"
            f"{sanitize_text(tomador)}\n"
            f"Atención: [ ]\n"
            f"Presente\n\n"
            f"Ref.: Oferta de Préstamo entre Clientes\n\n"
            f"De nuestra mayor consideración:\n"
            f"Conforme a las conversaciones mantenidas, nos dirigimos a {sanitize_text(prestamista)} (en adelante, el 'Prestamista'), a fin de formular con carácter de irrevocable la presente Oferta de Préstamo de Valores Negociables entre Clientes (en adelante, la 'Oferta de Préstamo').\n"
            ...
            f"Carácter:\n\n"
            f"ANEXO I\n"
            f"OFERTA DE PRÉSTAMO DE VALORES NEGOCIABLES ENTRE CLIENTES\n"
            f"En el supuesto de ser aceptada la Oferta de Préstamo en los términos aquí previstos, de la cual la presente forma parte como Anexo I, se entenderá que se ha perfeccionado el siguiente contrato de préstamo (en adelante, el 'Contrato de Préstamo' o el 'Contrato' indistintamente), y tendrá como partes a:\n"
            ...
            f"b) por la otra, {sanitize_text(tomador)}, CUIT {sanitize_text(cuit_tomador)}, con domicilio en {sanitize_text(domicilio_tomador)} (en adelante, el 'Tomador' y conjuntamente con el Prestamista, las 'Partes').\n\n"
            f"PRIMERA: El Prestamista transfiere al Tomador en calidad de préstamo, los siguientes valores negociables: {sanitize_text(especie)} por un valor nominal de {sanitize_text(valor_nominal)} ({sanitize_text(valor_nominal_texto)}) con el alcance y extensión que se detalla en el Anexo II (en adelante, los 'Valores Negociables').\n"
            f"SEGUNDA: El presente préstamo se establece por un plazo de {sanitize_text(plazo)} ({sanitize_text(plazo_texto)}) meses contados a partir de la transferencia de los Valores Negociables al Tomador (en adelante, el 'Plazo'). Durante el Plazo, el Prestamista percibirá una tasa de interés equivalente al {sanitize_text(tasa_anual)}% nominal anual que será abonado, en pesos, por el Tomador al vencimiento del Plazo.\n"
            f"TERCERA: La renovación de la vigencia del Contrato acaecerá de modo automático en ausencia de una notificación de cancelación anticipada conforme la cláusula CUARTA.\n"
            f"CUARTA: El Prestamista podrá solicitar la cancelación anticipada del préstamo de los Valores Negociables antes del vencimiento del Plazo, para lo cual deberá notificar por escrito al Tomador con una antelación de 48 horas hábiles a la efectiva fecha de cancelación. En caso de ejercerse tal derecho, el Tomador abonará en forma proporcional el importe de la contraprestación convenida en la cláusula SEGUNDA.\n"
            f"QUINTA: El Tomador se obliga a restituir los Valores Negociables al vencimiento del Plazo.\n"
            f"SEXTA: El Tomador se compromete a realizar todos aquellos actos necesarios para la conservación de los Valores Negociables, obligándose a restituirlos a la finalización del Plazo en igual cantidad y especie que los recibiera.\n"
            f"SÉPTIMA: El Tomador pagará todos los gastos que genere la operatoria objeto del presente Contrato.\n"
            f"OCTAVA: La obligación de restituir los Valores Negociables no requiere interpelación judicial o extrajudicial alguna, configurándose su incumplimiento de pleno derecho por el solo incumplimiento material de la obligación de que se trate en la fecha estipulada, dando derecho al Prestamista a considerar vencido el Plazo y exigir la inmediata cancelación del Contrato de Préstamo y de los intereses devengados bajo el mismo.\n"
            f"NOVENA: El tomador declara conocer que la falta de devolución de los Valores Negociables en tiempo y forma generará una penalidad del {sanitize_text(tasa_penalidad)}% mensual por cada día de retardo en el cumplimiento de su obligación de restituir.\n"
            ...
            f"\nANEXO II\n"
            f"Condiciones de la operación de Préstamo de Títulos Valores entre Clientes:\n"
            f"Prestamista: {sanitize_text(prestamista)}, CUIT {sanitize_text(cuit_prestamista)}, domicilio {sanitize_text(domicilio_prestamista)}.\n"
            f"Tomador: {sanitize_text(tomador)}, CUIT {sanitize_text(cuit_tomador)}, domicilio {sanitize_text(domicilio_tomador)}.\n"
            f"Especie: {sanitize_text(especie)}.\n"
            f"Valor Nominal: {sanitize_text(valor_nominal)} ({sanitize_text(valor_nominal_texto)}).\n"
            f"Tasa: {sanitize_text(tasa_anual)}% nominal anual.\n"
            f"Plazo: {sanitize_text(plazo)} ({sanitize_text(plazo_texto)}) meses.\n")
...
def generate_pdf_prestamo_entre_clientes_tbills(mes, dia, prestamista, tomador, cuit_prestamista, cuit_tomador, domicilio_prestamista, domicilio_tomador, especie, valor_nominal, tasa_anual, plazo, tasa_penalidad):
    pdf = PDF()
    pdf.add_page()
    pdf.set_left_margin(10)
    pdf.set_right_margin(10)
    ...
    body = (f"Ciudad Autónoma de Buenos Aires, {sanitize_text(dia)} de {sanitize_text(mes)} de 2024\n\n"
            ...
            f"Atención: [ ]\n"
            f"Presente\n\n"
            f"Ref.: Oferta de Préstamo entre Clientes T-Bills\n\n"
            f"De nuestra mayor consideración:\n"
            f"Conforme a las conversaciones mantenidas, nos dirigimos a {sanitize_text(prestamista)} (en adelante, el 'Prestamista'), a fin de formular con carácter de irrevocable la presente Oferta de Préstamo de Valores Negociables entre Clientes T-Bills (en adelante, la 'Oferta de Préstamo').\n"
            f"A los efectos de la presente Oferta de Préstamo, el Prestamista y {sanitize_text(tomador)} (en adelante, el 'Tomador') serán denominados en forma conjunta como las 'Partes'.\n"
            f"PRIMERO: El Tomador ofrece al Prestamista realizar un contrato de préstamo bajo el cual el Prestamista entregará al Tomador, en calidad de préstamo, los Valores Negociables que se indican en el Anexo I a la presente Oferta de Préstamo, bajo el cual se establecen los términos y condiciones que regirán dicho contrato.\n"
            f"SEGUNDO: En caso que el Prestamista decida aceptar la presente Oferta de Préstamo, las obligaciones y derechos de las Partes serán estrictamente los que resultan del Anexo I adjunto a la presente.\n"
            f"TERCERO: La presente Oferta de Préstamo tiene vigencia por el plazo de 5 (cinco) días hábiles, considerándose aceptada si en o antes de dicho plazo, el Prestamista realiza la transferencia de los Valores Negociables a la cuenta comitente del Tomador, conforme se establece en la cláusula PRIMERA del Anexo I adjunto.\n"
            ...
            f"Carácter:\n\n"
            f"ANEXO I\n"
            f"OFERTA DE PRÉSTAMO DE VALORES NEGOCIABLES ENTRE CLIENTES T-BILLS\n"
            f"En el supuesto de ser aceptada la Oferta de Préstamo en los términos aquí previstos, de la cual la presente forma parte como Anexo I, se entenderá que se ha perfeccionado el siguiente contrato de préstamo (en adelante, el 'Contrato de Préstamo' o el 'Contrato' indistintamente), y tendrá como partes a:\n"
            f"a) {sanitize_text(prestamista)}, CUIT {sanitize_text(cuit_prestamista)}, con domicilio en {sanitize_text(domicilio_prestamista)} (en adelante, el 'Prestamista'), por una parte, y\n"
            f"b) por la otra, {sanitize_text(tomador)}, CUIT {sanitize_text(cuit_tomador)}, con domicilio en {sanitize_text(domicilio_tomador)} (en adelante, el 'Tomador' y conjuntamente con el Prestamista, las 'Partes').\n\n"
            f"PRIMERA: El Prestamista transfiere al Tomador en calidad de préstamo, los siguientes valores negociables: {sanitize_text(especie)} por un valor nominal de {sanitize_text(valor_nominal)} ({sanitize_text(valor_nominal_texto)}) con el alcance y extensión que se detalla en el Anexo II (en adelante, los 'Valores Negociables').\n"
            ...
            f"DÉCIMO CUARTA: Para todos los efectos legales derivados de esta Oferta, las Partes constituyen sus domicilios en los indicados en el segundo párrafo del presente Anexo, donde se tendrán por válidas todas las notificaciones. Toda controversia relacionada al presente Contrato será resuelta en forma inapelable por el Tribunal de Arbitraje General de la Bolsa de Comercio de Buenos Aires por las reglas del arbitraje de derecho, que las partes declaran conocer y aceptar.\n"
            f"\nANEXO II\n"
            f"Condiciones de la operación de Préstamo de Títulos Valores entre Clientes T-Bills:\n"
            f"Prestamista: {sanitize_text(prestamista)}, CUIT {sanitize_text(cuit_prestamista)}, domicilio {sanitize_text(domicilio_prestamista)}.\n"
            f"Tomador: {sanitize_text(tomador)}, CUIT {sanitize_text(cuit_tomador)}, domicilio {sanitize_text(domicilio_tomador)}.\n"
            f"Especie: {sanitize_text(especie)}.\n"
            f"Valor Nominal: {sanitize_text(valor_nominal)} ({sanitize_text(valor_nominal_texto)}).\n"
...
def enviar_email(pdf_data, filename):
    ...
    try:
        ...
        servidor.sendmail(usuario_smtp, 'destinatario@example.com', mensaje.as_string())
        servidor.quit()
...
if st.button('Generar PDF'):
    ...
    if tipo_prestamo == 'COHEN TOMADOR':
        ...
        pdf_data = generate_pdf_cohen_tomador(mes, dia, prestamista, tasa_penalidad, prestamista, None, None, tomador, None, None, especie, None, valor_nominal, tasa_anual, plazo, None, cuit_prestamista, domicilio_prestamista)
    elif tipo_prestamo == 'COHEN PRESTAMISTA':
        filename = 'oferta_prestamo_cohen_prestamista.pdf'
        pdf_data = generate_pdf_cohen_prestamista(mes, dia, tomador, tasa_penalidad, prestamista, None, None, tomador, None, None, especie, None, valor_nominal, tasa_anual, plazo, None, cuit_tomador, domicilio_tomador)
    elif tipo_prestamo == 'COHEN TOMADOR T-BILLS':
        filename = 'oferta_prestamo_cohen_tomador_tbills.pdf'
        pdf_data = generate_pdf_cohen_tomador_tbills(mes, dia, prestamista, tasa_penalidad, prestamista, None, None, tomador, None, None, especie, None, valor_nominal, tasa_anual, plazo, None, cuit_prestamista, domicilio_prestamista)
    elif tipo_prestamo == 'COHEN PRESTAMISTA T-BILLS':
        filename = 'oferta_prestamo_cohen_prestamista_tbills.pdf'
        pdf_data = generate_pdf_cohen_prestamista_tbills(mes, dia, tomador, tasa_penalidad, prestamista, None, None, tomador, None, None, especie, None, valor_nominal, tasa_anual, plazo, None, cuit_tomador, domicilio_tomador)
    elif tipo_prestamo == 'PRESTAMO ENTRE CLIENTES':
        filename = 'oferta_prestamo_entre_clientes.pdf'
        pdf_data = generate_pdf_prestamo_entre_clientes(mes, dia, prestamista, tomador, cuit_prestamista, cuit_tomador, domicilio_prestamista, domicilio_tomador, especie, valor_nominal, tasa_anual, plazo, tasa_penalidad)
    elif tipo_prestamo == 'PRESTAMO ENTRE CLIENTES T-BILLS':
        filename = 'oferta_prestamo_entre_clientes_tbills.pdf'
        pdf_data = generate_pdf_prestamo_entre_clientes_tbills(mes, dia, prestamista, tomador, cuit_prestamista, cuit_tomador, domicilio_prestamista, domicilio_tomador, especie, valor_nominal, tasa_anual, plazo, tasa_penalidad)
